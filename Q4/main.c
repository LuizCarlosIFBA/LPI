...
void main(){
	...
	while(n>5 && n<=0){    
        ...
        int peso[n];
        float mediaFinal[TAM];
	float **nota;

	//aloco vetor principal
	nota = malloc((n) *sizeof(sizeof(float *)));
	
	//aloco cada vetor linha
	for(int i=0; i<n; i++){
		nota[i] = (float *)malloc((TAM)* sizeof(float));
	}
	
	//Ponteiro peso, média e nota
	int *pPeso = &peso[n]; 	
	float *pMediaFinal = &mediaFinal[TAM];


        AlunoDestaque(n,TAM,nota,pMediaFinal,pPeso);
}
